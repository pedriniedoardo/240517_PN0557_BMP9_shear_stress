...
list_genes <- list(list_UP = results$res_GMPvsRESEARCH_shr %>% filter(log2FoldChange>0) %>% pull(symbol),
    list_DOWN = results$res_GMPvsRESEARCH_shr %>% filter(log2FoldChange<0) %>% pull(symbol))
...
list_enrichr %>%
  write_tsv("../../out/table/enrichR_GMPvsRESEARCH_shr.tsv")
...
ggsave("../../out/image/enrichR_GMPvsRESEARCH_shr.pdf",width = 13,height = 25,limitsize = FALSE)
