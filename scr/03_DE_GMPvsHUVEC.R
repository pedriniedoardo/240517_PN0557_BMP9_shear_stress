...
# read in the data --------------------------------------------------------
ddsHTSeq_filter <- readRDS("../../out/object/dds_all_filter_GMPvsHUVEC.rds")
design <- readRDS("../../out/object/design_all_dds_GMPvsHUVEC.rds")
vds_filter <- readRDS("../../out/object/vds_all_filter_GMP_vs_HUVEC_ECFCs.rds")
...
saveRDS(ddsHTSeq_filter,"../../out/object/ddsHTSeq_filter_DESeq_GMPvsHUVEC.rds")
...
contrast <- makeContrasts(GMPvsHUVEC = conditionGMP,
                          levels = design)

res <- results(ddsHTSeq_filter, contrast=contrast[,"GMPvsHUVEC"],alpha = 0.05)
...
list_df$GMPvsHUVEC %>%
  ...
  theme_bw()
ggsave("../../out/image/histogram_pvalue_GMPvsHUVEC.pdf",width = 4,height = 3)

# PLOTTING RESULTS --------------------------------------------------------
# add the info of the genename
test_plot <- list_df$GMPvsHUVEC %>%
    data.frame() %>%
...
ggsave("../../out/image/vulcano_plot_text_GMPvsHUVEC.pdf",width = 12,height = 12)
#
...
#
test_plot_shr <- list_df_shr$GMPvsHUVEC_shr%>%
  data.frame()%>%
  # add a clor variable in case significant
  mutate(col=ifelse(((padj<0.05)&abs(log2FoldChange)>1&!is.na(symbol)),yes = 1,no = 0)) %>% 
  filter(!is.na(col))
...
test_plot_shr %>%
  ...
  theme_bw() +
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_text_GMPvsHUVEC_shr.pdf",width = 12,height = 12)
...
list_df$GMPvsHUVEC %>%
  ...
  theme(legend.position = "none")
ggsave("../../out/image/MA_plot_GMPvsHUVEC.pdf",width = 4,height = 3)

# using the shrinked values
list_df_shr$GMPvsHUVEC_shr %>%
  filter(!is.na(padj)) %>% 
  data.frame()%>%
  mutate(color = ifelse(padj<0.05,yes = 1,no = 0))%>%
  mutate(color = factor(ifelse(is.na(color),yes = 0,no = color)))%>%
  ...
  theme_bw() +
  theme(legend.position = "none")
ggsave("../../out/image/MA_plot_shr_GMPvsHUVEC.pdf",width = 12,height = 12)
...
# the DEGs plot
DEG_2 <- list_df_shr$GMPvsHUVEC_shr %>%
  data.frame()%>%
  # add a clor variable in case significant
  mutate(col=ifelse(((padj<0.05)&abs(log2FoldChange)>1),yes = 1,no = 0)) %>%
  dplyr::filter(col==1) %>%
...
column_ha_shr <- HeatmapAnnotation(treat = meta_sample$Condition,  
                                   col = list(treat = c("GMP" = "green", "HUVEC" = "black"))) 

ht2_shr <- Heatmap(mat2_shr, show_column_names = T,
                   name = "exp", 
                   column_title = "GMPvsHUVECshr",
                   # row_names_gp = gpar(fontsize = 3),
...
pdf("../../out/image/heatmap_DEG_shr_GMPvsHUVEC.pdf",width = 7,height = 8) 
draw(ht2_shr,heatmap_legend_side = "left",annotation_legend_side = "left") 
dev.off()

# PLOT DISPERSION ---------------------------------------------------------
pdf("../../out/image/ddsHTSeq_filter_dispersion_GMPvsHUVEC.pdf",width = 5,height = 5) 
