...
LUT_samples <- read_csv("../../data/lut_sample.csv") %>% 
    filter(Condition %in% c("GMP","HUVEC"))
...
# save the table
write.csv(coldata,file = "../../data/LUT_samples_final_GMPvsHUVEC.csv",row.names = T)
...
condition <- factor(coldata$Condition,levels = c("HUVEC","GMP"))
...
# save the raw object
saveRDS(object = dds,file = "../../out/object/dds_all_raw_GMPvsHUVEC.rds")
saveRDS(object = design,file = "../../out/object/design_all_dds_GMPvsHUVEC.rds")
