...
LUT_samples <- read_csv("../../data/lut_sample.csv") %>% 
    filter(Condition %in% c("GMP","Research"))
...
# save the table
write.csv(coldata,file = "../../data/LUT_samples_final_GMPvsResearch.csv",row.names = T)
...
condition <- factor(coldata$Condition,levels = c("Research","GMP"))
...
# save the raw object
saveRDS(object = dds,file = "../../out/object/dds_all_raw_GMPvsResearch.rds")
saveRDS(object = design,file = "../../out/object/design_all_dds_GMPvsResearch.rds")
