...
df_tables_GSEA_all %>%
  write_tsv("../../out/table/df_tables_GSEA_res_HYPvsNORM_shr_all_REACTOME.tsv")
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv(file = "../../out/table/df_table_GSEA_res_HYPvsNORM_shr_nonredundant_REACTOME.tsv")
...
ggsave("../../out/image/GSEA_unbiased_res_HYPvsNORM_shr_nonredundant_REACTOME.pdf",width = 7,height = 5)
...
df_tables_GSEA_all %>%
  ...
  theme(strip.background = element_blank())+
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)
ggsave("../../out/image/GSEA_unbiased_res_HYPvsNORM_shr_REACTOME.pdf",width = 15,height = 9)

# PLOT PROFILE ------------------------------------------------------------ 
# library("patchwork")
plot_pathway <- lapply(list_mainPathways$res_HYPvsNORM_shr[1:9],function(x){ 
  name <- str_sub(x,start = 1,end = -4) 
  # pdf(file = paste0("HUVEC_BMP9_24h_",name,".pdf"),width = 3,height = 3) 
  plotEnrichment(pathways[[x]], list_ranks$res_HYPvsNORM_shr) + labs(title = name) 
})

wrap_plots(plot_pathway)
ggsave(filename = "../../out/image/GSEA_plot_profile_HYPvsNORM_shr_nonredundant_REACTOME.pdf",width = 15,height = 10)
