...
df_tables_GSEA_all_non_redundant <- read_tsv(file = "../../out/table/df_tables_GSEA_res_GMPvsRESEARCH_shr_all_HALLMARK.tsv")
...
df_test |> 
  filter(str_detect(pathway,pattern = c("HALLMARK_GLYCOLYSIS")))
...
df_plot <- df_tables_GSEA_all_non_redundant %>%
  ...
  #                          T~"black")) %>%
  mutate(color = case_when(pathway %in% c("HALLMARK_GLYCOLYSIS")~"red",
                           T~"black")) %>%
...
ggsave("../../out/image/volcano_GSEA_HALLMARK_GMPvsRESEARCH_tailored.pdf",width = 8,height = 5)
...
list_hm <- lapply(c("HALLMARK_GLYCOLYSIS"),
                  function(pp){
...
pdf("../../out/image/heatmap_DEG_HALLMARK_GMPvsRESEARCH_leadingEdges.pdf",width = 7,height = 8)
